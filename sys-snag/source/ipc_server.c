...
Result ipcServerInit(IpcServer* server, const char* name, u32 max_sessions)
{
    ...
    SnagLog("ipcServerInit name=%s max=%u rc=0x%08X", name, (unsigned)max_sessions, rc);
    return rc;
}
...
static Result _ipcServerAddSession(IpcServer* server, Handle session)
{
    ...
    server->count++;
    SnagLog("accept session count=%u", (unsigned)server->count);
    return 0;
}
...
static Result _ipcServerDeleteSession(IpcServer* server, u32 index)
{
    ...
    SnagLog("delete session idx=%u count=%u", (unsigned)index, (unsigned)server->count);
...
static Result _ipcServerParseRequest(IpcServerRequest* r)
{
    ...
    if(r->hipc.meta.type == CmifCommandType_Request)
    {
        ...
        {
            SnagLog("parse FAIL dataSize=%u magic=0x%08X", (unsigned)dataSize, header ? (unsigned)header->magic : 0xFFFFFFFFu);
            return MAKERESULT(Module_Libnx, LibnxError_BadInput);
        }
...
static void _ipcServerPrepareResponse(Result rc, void* data, size_t dataSize)
{
    ...
    if(R_SUCCEEDED(rc) && data && dataSize > 0)
    {
...
static Result _ipcServerProcessSession(IpcServer* server, IpcServerRequestHandler handler, void* userdata, u32 handleIndex)
{
    ...
    if(R_SUCCEEDED(rc))
    {
        ...
        // NOTE: do NOT log here. TLS still holds the live request and any
        // IPC (e.g. fopen inside SnagLog) would clobber the buffer
        // descriptors before the handler reads them.
    }
    else
    {
        SnagLog("recv FAIL rc=0x%08X", rc);
    }
    ...
    if(R_SUCCEEDED(rc))
    {
        ...
        {
            SnagLog("reply FAIL rc=0x%08X outSize=%u", rc, (unsigned)dataSize);
        }
    }
