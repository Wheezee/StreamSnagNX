...
public:
    ...
    // True while any PCM buffer is still queued in the driver.
    bool     HasQueuedBuffers() const;
    uint32_t GetPlayedSampleCount() const { return 0; }
    bool     IsInitialized() const { return inited_; }

    // Optional abort flag (owned by PlayerDaemon). When set, a blocked
    // SubmitBuffer gives up within ~20 ms so track switches stay snappy.
    // Must outlive this engine (daemon-owned atomic).
