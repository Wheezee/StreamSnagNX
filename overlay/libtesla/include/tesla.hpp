...
namespace tsl {
    ...
    namespace cfg {
        ...
        constexpr u32 LayerMaxWidth = 1280;     ///< Reference width for layer sizing (Status Monitor pattern)
        constexpr u32 LayerMaxHeight = 720;     ///< Reference height for layer sizing
    ...
    namespace gfx {
        ...
        public:
            ...
            /**
             * @brief Moves the Tesla layer on screen (Status Monitor pattern)
             *
             * @param x X position
             * @param y Y position
             */
        ...
        private:
            ...
            void init() {
                ...
                cfg::LayerPosX = 0;
                cfg::LayerPosY = 0;
                // Patched for StreamSnagNX (was: unconditional 448x720).
                // Only default when the app didn't choose a size first:
                // main() may set a fullscreen (or strip) framebuffer before
                // tsl::loop() runs. Layer dims always follow the framebuffer.
                if (cfg::FramebufferWidth == 0 || cfg::FramebufferHeight == 0)
                {
                    cfg::FramebufferWidth  = 448;
                    cfg::FramebufferHeight = 720;
                }
                // Proportional layer sizing (Status Monitor pattern): a
                // 1280x64 strip becomes a lean 1920x96 layer instead of a
                // full 1920x1080 one. Keeps tiny overlays inside the heap
                // budget that killed our first fullscreen attempt (0x559).
